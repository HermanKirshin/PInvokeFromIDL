...
int CreateTrampolineFunction(HOOK_ENTRY* ct)
{
    ...
    while (!finished)
    {
        ...
        else if ((hs.opcode & 0xF0) == 0x70 || (hs.opcode & 0xFC) == 0xE0 || (hs.opcode2 & 0xF0) == 0x80)
        {
            ...
            else
            {
				...
				n = ((hs.opcode != 0x0F ? hs.opcode : hs.opcode2) & 0x0F);
                jcc.opcode = 0x70 | n;
    ...
    if (oldPos < sizeof(JMP_REL) && !IsCodePadding((unsigned char*)ct->pTarget + oldPos, sizeof(JMP_REL)-oldPos))
    {
        ...
        regionAddress = (unsigned char*)ct->pTarget - sizeof(JMP_REL);
        returnedLength;
...
int ResolveTemporaryAddresses(HOOK_ENTRY* ct)
{
    ...
    for (i = 0, count = ct->tempAddrSize; i < count; ++i)
    {
        void* addr;
...
void* AllocateBuffer(void* const pOrigin, unsigned int protect, unsigned long long size)
{
    ...
    if (pOrigin != 0)
    {
        ...
        for (i = 0; i < (max - min + 1); ++i)
        {
            ...
            rel = -rel + (i & 1);
            pQuery = (void*)(((min + max) / 2 + rel) * blockSize);
...
int CreateHook(HOOK_ENTRY* pHook, void* pTarget, void* const pDetour, void** ppOriginal)
{
    ...
    do
    {
		...
		int trampolineSize, tableSize, backupAlignedSize, pRelayAlignedSize;
        ...
        trampolineSize = ALIGNED_SIZE(pHook->trampolineSize);
        ...
        tableSize = ALIGNED_SIZE((pHook->tableSize + 1) * sizeof(void*));
        ...
        backupAlignedSize = ALIGNED_SIZE(backupSize);
        ...
        pRelayAlignedSize = ALIGNED_SIZE(sizeof(JMP_ABS));
        ...
        *((void**)pHook->pTable + pHook->tableSize) = pDetour;
        ...
        regionSize = (void*)tableSize;
        regionAddress = pHook->pTable;
